...
function timeAgo(v){ var t=ts(v); if(!t) return 'ახლახან'; var s=Math.max(1, Math.floor((Date.now()-t)/1000)); if(s<60)return s+'s'; var m=Math.floor(s/60); if(m<60)return m+'m'; var h=Math.floor(m/60); if(h<24)return h+'h'; var d=Math.floor(h/24); if(d<30)return d+'d'; var mo=Math.floor(d/30); if(mo<12)return mo+'mo'; return Math.floor(mo/12)+'y'; }
      ...
      gs.addEventListener('input',function(){
        ...
        _gsTimer=setTimeout(function(){
          ...
          geo.searchFirestore(q,function(res){
            ...
            (res.places||[]).slice(0,2).forEach(function(p){items.push({type:'place',icon:'fa-map-marker-alt',avatar:p.imageUrl||p.photoUrl||p.coverUrl||p.coverImageUrl||'',label:p.name||'Place',sub:p.address||p.category||p.city||'Place',href:'places.html'});});
  ...
  function renderGroupTab(g){
    ...
    listenTargetPosts('group', g.id, function(items){
  ...
  function renderGroupMembersTab(g,box,isAdmin){
    ...
    GS().listenGroupMembers(g.id,function(items){allMembers=items;paintMembers();});
  }
  ...
  function renderGroupAdminTab(g,box){
    ...
    state.pageUnsubs.push(ju);
    GS().getGroupInsights(g.id,function(data){var ins=$('#ghGroupInsights');if(!ins)return;if(!data){ins.innerHTML='<p class="gh-muted">Could not load insights.</p>';return;}ins.innerHTML='<div class="gr-insights-grid"><div class="gr-insight-card"><div class="gr-insight-val">'+Number(data.memberCount||0)+'</div><div class="gr-insight-label">Total Members</div></div><div class="gr-insight-card"><div class="gr-insight-val">'+Number(data.postCount30||0)+'</div><div class="gr-insight-label">Posts (30 days)</div></div></div>'+(data.topContributors&&data.topContributors.length?'<div style="margin-top:14px"><h4 style="margin-bottom:8px;font-size:.9rem">Top Contributors</h4>'+data.topContributors.map(function(c,i){return '<div class="gr-contributor-row"><span>#'+(i+1)+'</span><span class="gh-chip sm">'+c.count+' posts</span></div>';}).join('')+'</div>':'');});
  }
